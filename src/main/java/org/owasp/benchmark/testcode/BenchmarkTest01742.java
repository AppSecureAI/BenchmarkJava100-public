...
public class BenchmarkTest01742 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Prepend IV to the encrypted result for decryption compatibility
            byte[] ivAndResult = new byte[iv.length + result.length];
            System.arraycopy(iv, 0, ivAndResult, 0, iv.length);
            System.arraycopy(result, 0, ivAndResult, iv.length, result.length);
            ...
            fw.write(
                    "secret_value="
                            + org.owasp.esapi.ESAPI.encoder().encodeForBase64(ivAndResult, true)
                            + "
");
            fw.close();
            response.getWriter()
